...
- [ ] Добавить и стабилизировать:
  ...
  - [ ] `ISystemEnd`
  - [ ] `ISystemDestroy`
- [ ] Добавить fixed update phase.
- [ ] Добавить render submit phase.
- [ ] Добавить optional late phase.
- [ ] Гарантировать deterministic execution order.
- [ ] Добавить clear error diagnostics.

### System Dependency Graph

- [ ] Поддержать lifecycle phases.
- [ ] Поддержать `RunAfter`.
- [ ] Поддержать `RunBefore`.
- [ ] Добавить cycle detection.
- [ ] Интегрировать с DragonECS runner.
- [ ] Добавить diagnostics for invalid order.
- [ ] Добавить visualization/debug dump of execution order.

### ECS State Rules

- [ ] Добавить правило: gameplay/runtime state только в `struct` components.
- [ ] Добавить правило: services не должны быть authoritative gameplay state.
- [ ] Добавить запрет/предупреждение для managed references в runtime components.
- [ ] Добавить документацию с примерами: правильно, неправильно, допустимые исключения.

### Module System

- [ ] Добавить module metadata/config:
  - [ ] name;
  - [ ] version;
  - [ ] required/optional;
  - [ ] enabled/disabled;
...
- [ ] Configurator должен валидировать:
  - [ ] project references;
  - [ ] side boundaries;
  - [ ] module graph;
  ...
  - [ ] invalid Client/Server references;
  - [ ] invalid Shared references.
- [ ] Поддержать зависимости модулей через `.csproj` project references.
- [ ] Добавить validation command.
- [ ] Выдавать clear errors before runtime.
- [ ] Добавить optional generated module manifest.

### Done Criteria

- [ ] Можно подключать/отключать модули.
- [ ] Порядок систем предсказуем.
- [ ] Lifecycle стабилен.
- [ ] Client/Server/Shared boundaries проверяются.
- [ ] Hot reload state rules задокументированы.
- [ ] Есть минимальные тесты module graph/system graph.
