...
public class Renderer
{
    ...
    public void Render(UIElement root, IFont font)
    {
    ...
    private void RenderContext(StackingContext context, IFont font)
    {
    ...
    private void RenderElementVisuals(UIElement element, IFont font)
    {
        var style = element.ComputedStyle;
        var box = element.LayoutBox;
    
        bool hasOverflowHidden = style.GetValueOrDefault("overflow") == "hidden";
        if (hasOverflowHidden) _renderer.BeginScissorMode(box.PaddingRect);

        var bgColor = ParseColor(style.GetValueOrDefault(s.background_color, s.transparent));
        if (bgColor.A > 0) _renderer.DrawRectangle(box.PaddingRect, bgColor);
        ...
        if (borderColor.A > 0)
        {
            ...
            if (borderWidth > 0)
                _renderer.DrawRectangleLines(box.BorderRect, borderWidth, borderColor);
        }
        ...
        if (element.WrappedTextLines.Any())
        {
            ...
            for (int i = 0; i < element.WrappedTextLines.Count; i++)
            {
                ...
                {
                    float textWidth = _renderer.MeasureText(font, line, fontSize, 1).X;
                ...
                _renderer.DrawText(font, line, position, fontSize, 1, textColor);
            }
        }
    
        if (hasOverflowHidden) _renderer.EndScissorMode();
    }
