...
public class LayoutEngine
{
    ...
    private void Calculate(UIElement parent, RectangleF availableSpace, IFont font)
    {
        ...
        var paddingLeft =
            ParseValue(style.GetValueOrDefault(s.padding_left, "0"))
                .ToPx(sizingBlock.Width); // padding % зависит от ширины родителя
        ...
        parent.LayoutBox.ContentRect = new RectangleF(finalX + borderLeft + paddingLeft,
            finalY + borderTop + paddingTop, contentWidth, finalContentHeight);
        ...
        if (heightVal.Unit == Unit.Auto)
        {
            ...
            parent.LayoutBox.ContentRect = new RectangleF(parent.LayoutBox.ContentRect.X,
                parent.LayoutBox.ContentRect.Y, contentWidth, finalContentHeight);
            RecalculateOuterRects(parent, marginLeft, marginRight, marginTop, marginBottom, paddingLeft,
                paddingRight, paddingTop, paddingBottom, borderLeft, borderRight, borderTop, borderBottom);
        }
    ...
    private float LayoutChildren(UIElement parent, RectangleF parentAvailableSpace, IFont font)
    {
    ...
    private float CalculateBlock(UIElement parent, IFont font)
    {
        float currentY = 0;
        foreach (var child in parent.Children)
        {
            ...
            var availableSpaceForChild = new RectangleF(
    ...
    private float CalculateFlex(UIElement parent, RectangleF parentAvailableSpace, IFont font)
    {
        ...
        var allItemData = flexItems.Select(item =>
        {
            ...
            {
                var availableSpaceForMeasure = new RectangleF(
                    0, 0,
        ...
        else
        {
            ...
            {
                if (currentLine.Items.Any() && currentLine.TotalFlexBasis + item.FlexBasis > mainSize &&
                    mainSize > 0)
                {
        ...
        foreach (var line in allLines)
        {
            ...
            if (freeSpace > 0)
            {
                float totalGrow = line.Items.Sum(d =>
                    ParseFloat(d.Element.ComputedStyle.GetValueOrDefault(s.flex_grow, "0")));
                foreach (var item in line.Items)
                {
                    float growFactor = ParseFloat(item.Element.ComputedStyle.GetValueOrDefault(s.flex_grow, "0"));
                    item.FinalMainSize =
                        item.FlexBasis + (totalGrow > 0 ? (growFactor / totalGrow) * freeSpace : 0);
                }
            }
            else
            {
                float totalWeightedShrink = line.Items.Sum(d =>
                    ParseFloat(d.Element.ComputedStyle.GetValueOrDefault(s.flex_shrink, "1")) * d.FlexBasis);
                foreach (var item in line.Items)
                {
                    float shrinkFactor =
                        ParseFloat(item.Element.ComputedStyle.GetValueOrDefault(s.flex_shrink, "1"));
                    item.FinalMainSize = item.FlexBasis + (totalWeightedShrink > 0
                        ? ((shrinkFactor * item.FlexBasis) / totalWeightedShrink) * freeSpace
                        : 0);
                }
            }
        ...
        foreach (var line in allLines)
        {
            ...
            foreach (var item in line.Items)
            {
                var tempAvailableSpace = new RectangleF(0, 0, isRow ? item.FinalMainSize : contentBox.Width,
                    isRow ? mainSize : item.FinalMainSize);
                Calculate(item.Element, tempAvailableSpace, font);
                float itemCrossSize = isRow
                    ? item.Element.LayoutBox.MarginRect.Height
                    : item.Element.LayoutBox.MarginRect.Width;
        ...
        if (freeCrossSpace > 0.01f && allLines.Any())
        {
            ...
            switch (alignContent)
            {
                ...
                case s.align_content_space_between:
                    if (allLines.Count > 1) lineSpacing = freeCrossSpace / (allLines.Count - 1);
                    break;
                ...
                case s.align_content_stretch:
                    if (allLines.Count > 0)
                    {
                        foreach (var l in allLines) l.CrossSize += freeCrossSpace / allLines.Count;
                    }
            ...
            foreach (var item in line.Items)
            {
                ...
                if (alignSelf == s.align_stretch)
                {
                    if (isRow)
                    {
                        ...
                        if (heightVal.Unit == Unit.Auto)
                        {
                            var mTop = ParseValue(itemStyle.GetValueOrDefault(s.margin_top, "0"))
                                .ToPx(crossSizeForSizing);
                            var mBottom = ParseValue(itemStyle.GetValueOrDefault(s.margin_bottom, "0"))
                                .ToPx(crossSizeForSizing);
                            var bTop = ParseValue(itemStyle.GetValueOrDefault(s.border_top_width, "0")).ToPx(0);
                            var bBottom = ParseValue(itemStyle.GetValueOrDefault(s.border_bottom_width, "0"))
                                .ToPx(0);
                            ...
                            float newContentHeight =
                                Math.Max(0, newBorderBoxHeight - bTop - bBottom - pTop - pBottom);

                            itemElement.LayoutBox.ContentRect = new RectangleF(itemElement.LayoutBox.ContentRect.X,
                                itemElement.LayoutBox.ContentRect.Y, itemElement.LayoutBox.ContentRect.Width,
                                newContentHeight);

                            var mLeft = ParseValue(itemStyle.GetValueOrDefault(s.margin_left, "0")).ToPx(mainSize);
                            var mRight = ParseValue(itemStyle.GetValueOrDefault(s.margin_right, "0"))
                                .ToPx(mainSize);
                            ...
                            RecalculateOuterRects(itemElement, mLeft, mRight, mTop, mBottom, pLeft, pRight, pTop,
                    ...
                    else
                    {
                        ...
                        if (widthVal.Unit == Unit.Auto)
                        {
                            var mLeft = ParseValue(itemStyle.GetValueOrDefault(s.margin_left, "0"))
                                .ToPx(crossSizeForSizing);
                            var mRight = ParseValue(itemStyle.GetValueOrDefault(s.margin_right, "0"))
                                .ToPx(crossSizeForSizing);
                            var bLeft = ParseValue(itemStyle.GetValueOrDefault(s.border_left_width, "0")).ToPx(0);
                            ...
                            float newContentWidth =
                                Math.Max(0, newBorderBoxWidth - bLeft - bRight - pLeft - pRight);

                            itemElement.LayoutBox.ContentRect = new RectangleF(itemElement.LayoutBox.ContentRect.X,
                                itemElement.LayoutBox.ContentRect.Y, newContentWidth,
                                itemElement.LayoutBox.ContentRect.Height);
                ...
                if (alignSelf == s.align_flex_end) crossPos = lineStartOffset + line.CrossSize - itemCrossSize;
                    ...
                    crossPos = lineStartOffset + (line.CrossSize - itemCrossSize) / 2;
    ...
    private (float width, float height) CalculateNaturalSize(UIElement element, IFont defaultFont,
        float? wrapWidth = null)
    {
        ...
        if (element.WrappedTextLines.Any())
        {
            textWidth = element.WrappedTextLines.Max(line =>
                _renderer.MeasureText(defaultFont, line, fontSize, 1).X);
            var lineHeight = ParseValue(style.GetValueOrDefault(s.line_height, "auto"))
        ...
        float bLeft, float bRight, float bTop, float bBottom)
    {
        ...
        element.LayoutBox.PaddingRect = new RectangleF(contentRect.X - pLeft, contentRect.Y - pTop,
            contentRect.Width + pLeft + pRight, contentRect.Height + pTop + pBottom);
        element.LayoutBox.BorderRect = new RectangleF(element.LayoutBox.PaddingRect.X - bLeft,
            element.LayoutBox.PaddingRect.Y - bTop, element.LayoutBox.PaddingRect.Width + bLeft + bRight,
            element.LayoutBox.PaddingRect.Height + bTop + bBottom);
        element.LayoutBox.MarginRect = new RectangleF(element.LayoutBox.BorderRect.X - mLeft,
            element.LayoutBox.BorderRect.Y - mTop, element.LayoutBox.BorderRect.Width + mLeft + mRight,
    ...
    private StyleValue ParseValue(string value)
    {
        ...
        {
            if (float.TryParse(value.AsSpan(0, value.Length - 2), NumberStyles.Any, CultureInfo.InvariantCulture,
                    out float pxVal)) return StyleValue.Px(pxVal);
        }

        if (value.EndsWith("%"))
        {
            if (float.TryParse(value.AsSpan(0, value.Length - 1), NumberStyles.Any, CultureInfo.InvariantCulture,
                    out float percentVal)) return StyleValue.Percent(percentVal);
        }
    ...
    private void WrapText(UIElement e, float maxWidth, IFont font, float fontSize)
    {
        ...
        foreach (var word in words)
        {
            ...
            if (_renderer.MeasureText(font, testLine, fontSize, 1).X > maxWidth && line.Length > 0)
            {
