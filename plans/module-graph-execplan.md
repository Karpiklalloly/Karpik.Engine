...
- [x] (2026-06-02) Remove the rejected Rider-plugin prototype from `Tools/KarpikModuleDependencies` and uninstall its local Rider package. The module-graph ExecPlan is complete.
...
This ExecPlan is complete. The repository uses shorthand `KarpikModuleDependency` ids, tracked catalog expansion, Configurator validation, deterministic generated artifacts, and deterministic installer lifecycle ordering. The optional Rider authoring plugin was rejected and removed after it failed manual IDE smoke tests.

Developers add a new dependency by editing the consuming `.csproj` directly:

```xml
<KarpikModuleDependency Include="Physics2D" />
```

Run Configurator generation only after adding, removing, or moving projects. Adding an existing catalog id to another project requires only a project reload or build.

### What to do next

Start the next engine feature as a separate task or ExecPlan. Do not continue extending this module-graph plan unless a regression is found in the accepted dependency contract.

For routine module-graph maintenance:

- add dependencies through `KarpikModuleDependency`, never direct `ProjectReference`, under `Modules` or `MyGame`;
- run `dotnet run --project Configurator\Configurator.csproj -- --generate` after project topology changes;
- run `dotnet run --project Configurator\Configurator.csproj -- --validate` before committing topology changes;
- use a separate ExecPlan if Rider authoring integration is revisited, with an automated IDE-level test harness as an entry requirement.
...
`Karpik.Engine.Core.Runner/Runner.cs` sorts installers by `[Module(priority)]`, then runs service registration and configuration callbacks. It destroys modules in reverse init order.
